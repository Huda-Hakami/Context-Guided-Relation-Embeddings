...
class SemEval:
    ...
    def load_dataset(self):
        ...
        for Q in self.data:
            ...
            Q["wpairs"] = [tuple(map(str.strip, line.strip().replace('"', '').split(':'))) for line in wpair_file]
            wpair_file.close()
        pass
    ...
    def get_correlation(self, fname, file_id):
        """
        Evaluate the result (Spearman Correlation). 
        """
        corr = None
        subprocess.call("sh semeval.sh %s %s %s > /dev/null" % (fname, file_id, self.data_path), shell=True)
        F = open("work/semeval-tmp/SpearmanRandomScaled-%s.txt" % file_id)
        for line in F:
            if line.startswith("Spearman Correlation:"):
                corr = float(line.strip().split(':')[1].split('%')[0])
        F.close()
        if corr is None:
            raise ("Could not read correlation from file = %s" % fname, ValueError)
        return corr
